...
static uint8_t SPI_CS = 0xff; 

static volatile Motion_Data Lastest_Data = {
    .x_dist = 0, //in cm
    .y_dist = 0, //in cm
    .timestamps = 0, // in ms
};
static volatile int dx = 0; //in cm
static volatile int dy = 0; //in cm
...
void pmw3360_spi_init(int16_t GPIO_CS, int16_t GPIO_SCLK, int16_t GPIO_MISO, int16_t GPIO_MOSI){
    ...
    ESP_LOGI(TAG, "ESP32 device initialized on SPI with MISO=%d, MOSI=%d, CLK=%d, CS=%d, HZ=%d", GPIO_MISO, GPIO_MOSI, GPIO_SCLK, GPIO_CS, PMW_3360_SPI_Frequency);
}
...
static uint8_t spi_read_reg(const uint8_t reg_addr){
    // printf("Read From %x\n",reg_addr & 0x7f);
...
static void spi_write_reg(const uint8_t reg_addr, uint8_t data)
{
...
void performStartup(){
    ...
    spi_write_reg(Shutdown, 0xb6); // force reset
    delay_ms(300);
    ...
    spi_read_reg(Revision_ID);
    spi_read_reg(Inverse_Product_ID);
...
void setCPI(int cpi)
{
    ...
    spi_write_reg(Config1, cpival);
    uint8_t rx = spi_read_reg(Config1);
...
void upload_firmware(){
    ...
    uint16_t Product_ID_read = spi_read_reg(Product_ID);
    printf("Got Product_ID %x\n", Product_ID_read);
    //Write 0x00 to Config2 register for wired mouse or 0x20 for wireless mouse design.
    spi_write_reg(Config2, 0x00);
}

static double CPI2CM(int16_t value){
    //Convert CPI to Inches, then convert Inches to Center meter
    double inches = value / (CPI * 1.0) ;
...
void data_collection_task(void* arg){
    ...
    for(;;){
        ...
        if(motion & (dx!=0) & (dy!=0)){
            Lastest_Data.x_dist += CPI2CM(dx);
            Lastest_Data.y_dist += CPI2CM(dy);
            // printf("dx: %d, dy: %d, %f, %f\n", dx, dy, Lastest_Data.x_dist, Lastest_Data.y_dist);
        }
        Lastest_Data.timestamps = endTime - startTime; 
        // printf("%.2f dx: %d, dy: %d, %f, %f\n",Lastest_Data.timestamps/100.0, dx, dy, Lastest_Data.x_dist, Lastest_Data.y_dist);
